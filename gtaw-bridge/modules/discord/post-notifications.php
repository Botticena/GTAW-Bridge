...
function gtaw_enqueue_discord_post_notification_assets($hook) {
    ...
    $preview_src = ( defined( 'GTAW_BRIDGE_PLUGIN_DIR' ) && is_readable( GTAW_BRIDGE_PLUGIN_DIR . 'assets/img/post-preview.webp' ) )
        ? ( defined( 'GTAW_BRIDGE_PLUGIN_URL' ) ? GTAW_BRIDGE_PLUGIN_URL . 'assets/img/post-preview.webp' : '' )
        : '';
    wp_localize_script(
        'gtaw-discord-post-notifications',
        'gtaw_discord_post_notifications',
        array(
...
function gtaw_add_enhanced_discord_notification_test_button($post) {
    ...
    <div class="misc-pub-section">
        ...
        <a href="#" id="test-discord-notification" class="button button-small" data-nonce="<?php echo esc_attr( $nonce ); ?>" data-post-id="<?php echo (int) $post->ID; ?>">Test Send</a>
...
jQuery(function($) {
	$('#test-discord-notification').on('click', function(e) {
		e.preventDefault();
		var $button = $(this);
		var $status = $('#discord-notification-status');
...
JS;
    wp_add_inline_script( 'jquery', $test_inline, 'after' );
}
